...
\centering
\includegraphics[width=\linewidth]{../outputs/plots/zipf.png}
\end{columns}
\end{frame}
...
\centering
\includegraphics[width=\linewidth]{../outputs/plots/heaps.png}
\end{columns}
\end{frame}
...
%------------------------------------------------
\begin{frame}{Sentence Segmentation (Rule-based)}
...
Do \textbf{NOT} split when:
\[
\text{decimal: } \quad \texttt{3.14},\ \texttt{50.5}
\]
\[
\text{initial: } \quad \texttt{A. Məlikli},\ \texttt{S.Rustamov}
\]
\[
\text{abbrev: } \quad \texttt{prof.},\ \texttt{Dr.}
\]

Split when:
\[
\text{quote close} + \text{space} + \text{Uppercase}
...
%------------------------------------------------
\begin{frame}{Segmentation Struggles (Real Wikipedia Text)}
\begin{itemize}
  \item Period does not always mean sentence end:
  \begin{itemize}
    \item initials: \texttt{S.Rustamov}
    \item abbreviations: \texttt{prof.}
    \item decimals: \texttt{2.5 dəfə}
  \end{itemize}
  \item Quotes and guillemets: punctuation inside quotation marks
  \item Lowercase continuation after punctuation (avoid over-splitting)
  \item Trade-off: fewer false splits vs missing some true boundaries
\end{itemize}
\end{frame}
...
\column{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{../outputs/spellcheck/confusion_heatmap.png}
\end{columns}
\end{frame}
