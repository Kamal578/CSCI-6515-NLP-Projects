...
We implemented a rule-based sentence segmenter (\texttt{src/sentence\_segment.py}) that applies contextual heuristics to punctuation-based splitting. It was run on the first 500 documents, producing 11,479 sentences (\texttt{outputs/sentences.txt}).
\begin{itemize}
    \item Blocks false splits on in-token punctuation (decimals like ``3.14'', initials like ``S.Rustamov'') and known abbreviations.
    \item Quote-aware: closing quote + space + lowercase $\Rightarrow$ no split; closing quote + space + uppercase $\Rightarrow$ split.
    \item Handles lowercase continuations after a period (e.g., ``kv. verst'') to prevent spurious boundaries.
    \item Regression tests cover abbreviations (\texttt{prof.}), decimals, initials, and lowercase continuations.
\end{itemize}
...
\subsection{Spell Checking}
The spell checker combines corpus-derived vocabulary filtering, length-aware candidate pruning, and (optional) weighted edit distance. Accuracy on 1,000 synthetic misspellings: Accuracy@1 = \spellaccone, Accuracy@5 = \spellaccfive. A confusion matrix of top substitutions is visualized in Figure~\ref{fig:confusion}, highlighting common character substitutions.

\paragraph{Candidate generation and scoring}
\begin{itemize}
    \item Vocab: filtered to Azerbaijani alphabet, min\_freq (default 2) and min\_len (default 3) to suppress noise.
    \item Pruning: length filter $|\,|w|-|c|\,|\leq$ max\_dist (default 2) before distance computation.
    \item Distance: uniform Levenshtein (\texttt{src/levenshtein.py}) or weighted edit distance (\texttt{src/weighted\_levenshtein.py}) if confusion weights are supplied.
    \item Ranking: sort by (distance asc, frequency desc), return top-$k$ (default 5).
    \item Confusion weights: learned from synthetic misspellings via \texttt{src/confusion.py}; common substitutions (e.g., missing diacritics) incur lower cost, improving ranking of realistic typos.
    \item Rare-word suggestions: \texttt{scripts/run\_all.sh} builds a list of 200 rare tokens and runs suggestions to spot likely errors automatically.
\end{itemize}
